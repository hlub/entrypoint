...
def process_templates(variables, output_root, template_root, jinja_root=None):
    ...
    """
    log.debug("Processing templates from %r.", template_root)
    template_roots = [template_root]
    if jinja_root:
        log.debug("Templates can be included from %r", jinja_root)
    ...
    for template_dir, _, files in os.walk(template_root):
        ...
        for file in files:
            ...
            if os.path.exists(output_file):
                log.warning(
                    "File %r has a template defined but already exists, "
                    "not overriding",
                    output_file,
                )
                continue
            log.debug("Processing template %r", template_file)
