...
def collect_variables(options):
    ...
    if os.path.exists(options.variables_file):
        if os.path.isdir(options.variables_file):
            raise RuntimeError('Problem opening configuration volume '
...
def parse_args(args=None):
    ...
    parser.add_argument('-q', '--quiet', action='store_true',
                        help='output only pure errors')
    parser.add_argument('--no-init', dest='dumb_init',
                        default=True, action='store_false',
                        help='disable dumb init functionality')
    parser.add_argument('--no-setsid', dest='use_setsid',
                        default=True, action='store_false',
                        help='omit use of setsid system call')
    parser.add_argument('-r', '--rewrite', metavar='SOURCE_SIG:DEST_SIG',
                        dest='rewrites', default=[], action='append',
                        help='specify signal rewrites using the signal names')
    parser.add_argument('-V', '--variables', metavar='PATH',
                        dest='variables_file', default='/variables.yml',
                        help='optional YAML file containing template variables')
    parser.add_argument('-t', '--templates', metavar='PATH',
                        dest='template_root', default='/templates',
                        help='directory structure containing template files')
    parser.add_argument('-j', '--jinja', metavar='PATH',
                        dest='jinja_root', default='/jinja',
    ...
    parser.add_argument('-H', '--hooks', metavar='PATH',
                        dest='hooks_root', default='/entrypoint_hooks',
                        help='directory containing entrypoint hooks '
                             'to run before the command')
...
def print_jinja_error(exc):
    ...
    source.
    """
    print('Jinja syntax error in {}:{}: {}'.format(exc.filename, exc.lineno, exc.message))
...
def main(args=None):
    ...
    try:
        ...
        if not os.environ.get('SKIP_ENTRYPOINT'):
            ...
            templates.process_templates(variables,
        ...
        if options.dumb_init:
            dumb_init.init(map(lambda arg: arg.split(':'), options.rewrites),
