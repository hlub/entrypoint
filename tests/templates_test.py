...
def test_copymode(mock_copymode, mock_chown, mock_stat):
    ...
    assert mock_chown.call_args[0][0] == "dst"
...
def test_make_output_dirs(copymode, mkdir, exists):
    ...
    assert mkdir.call_args_list == [call("/etc"), call("/etc/nginx")]
    ...
    assert mkdir.call_args_list == [call("/etc/nginx"), call("/etc/nginx/conf.d")]
    assert copymode.call_args_list == [
        call("/templates/etc/nginx", "/etc/nginx"),
...
def test_jinja_do_extension_enabled():
    ...
    out = render_string(
        '{%- do some_dict.update({"var": 1}) -%}' "{{ some_dict.var }}",
        {"some_dict": {}},
    )
    assert out == "1"
...
def test_process_templates(walk, stat, exists, mkdir, open, copymode):
    ...
    process_templates(
        {}, output_root="/", template_root="/templates", jinja_root="/jinja"
    )
