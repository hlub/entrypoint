...
def test_unique():
    ...
    out = render_string(
        "{{ values | unique | sort }}", {"values": [{"x": 1}, {"x": 1}]}
    )
    assert out == """[{'x': 1}]"""
...
def test_union():
    ...
    assert out == "{1, 2, 3}"
    out = render_string(
        "{{ set1 | union(set2) }}", {"set1": {1, 2, 3, 4}, "set2": {2, 3}}
    )
    assert out == "{1, 2, 3, 4}"
    out = render_string(
        "{{ set1 | union(set2) }}", {"set1": [{"x": 0}], "set2": [{"x": 1}]}
    )
    assert out == """[{'x': 0}, {'x': 1}]"""
    out = render_string(
        "{{ set1 | union(set2) }}", {"set1": [1, 2, 3, 4], "set2": [2, 3]}
...
def test_intersect():
    ...
    out = render_string(
        "{{ set1 | intersect(set2) }}", {"set1": {1, 2, 3, 4}, "set2": set()}
    )
    assert out == "set()"
    out = render_string(
        "{{ set1 | intersect(set2) }}", {"set1": {1, 2, 3, 4}, "set2": {2, 3}}
    )
    assert out == "{2, 3}"
    out = render_string(
        "{{ set1 | intersect(set2) }}",
        {"set1": [[0, 1], [1, 2]], "set2": [[2, 3], [1, 2]]},
    )
    assert out == "[[1, 2]]"
...
def test_difference():
    ...
    out = render_string(
        "{{ set1 | difference(set2) }}", {"set1": {1, 2, 3, 4}, "set2": {4, 5}}
    )
    assert out == "{1, 2, 3}"
