...
SUSPEND_SIGNALS = frozenset([signal.SIGTSTP, signal.SIGTTOU, signal.SIGTTIN,])
...
def print_signals(args=()):
    ...
    proc = popen_entrypoint(
        tuple(args) + ("--", sys.executable, "-m", "tests.utils.print_signals")
    )
...
def pid_tree(pid):
    ...
    return {pid for child in children for pid in pid_tree(child)} | children
